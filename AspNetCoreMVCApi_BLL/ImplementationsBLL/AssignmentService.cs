...
using AspNetMVCCoreApi_DAL.Contracts;
using AutoMapper;
using System;
        ...
        public IResult Delete(int assignmentId)
        {
            try
            {
                if (assignmentId > 0)
                {
                    ...
                    if (assignment == null)
                    {
                        throw new Exception("HATA : Görev bulunamadığı için silme başarısızdır");
                    }
                  
                    var result = _unitOfWork.AssignmentRepo.Delete(assignment);
                    if (!result)
                    {
                        return new ErrorResult("HATA : Görev beklenmedik bir hata nedeniyle görev silme işlemi başarısızdır!");
                    }
